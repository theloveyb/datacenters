"""The Behar Group scraper.

The Behar Group (behargroup.com) is a Toronto-based CRE brokerage.
Likely a smaller site with static listing pages.
"""
...
class BeharScraper(BaseScraper):
    BROKERAGE_NAME = "The Behar Group"
    BASE_URL = "https://www.behargroup.com"
    ...
    def scrape(self) -> list[dict]:
        ...
        listing_urls = [
            ...
            f"{self.BASE_URL}/featured-properties",
            f"{self.BASE_URL}/for-sale",
            f"{self.BASE_URL}/for-lease",
        ]
        ...
        for url in listing_urls:
            try:
                ...
                self.logger.warning(f"Behar page {url} failed: {e}")

        if not found_page:
            try:
                soup = fetch_page(self.BASE_URL, self.session)
                if soup:
                    # Look for property/listing navigation links
                    nav_links = soup.select("nav a, .menu a, .nav a")
                    for link in nav_links:
                        href = link.get("href", "")
                        text = link.get_text(strip=True).lower()
                        if any(kw in text for kw in ["propert", "listing", "available", "sale", "lease"]):
                ...
                self.logger.error(f"Behar homepage scrape failed: {e}")
    ...
    def _parse_page(self, soup, page_url: str) -> list[dict]:
        ...
        cards = soup.select(
            ...
            ".listing-item, article, .property, .listing, "
            ".properties-list > div, .grid-item, .property-box, "
            ".property-listing, .featured-property"
        )
        ...
        for card in cards:
            try:
                ...
                # Skip non-listing content
                if any(skip in address.lower() for skip in ["about", "contact", "team", "blog", "news"]):
                    continue

                text = card.get_text(" ", strip=True)

                type_el = card.select_one(".property-type, .type, .category")
                price_el = card.select_one(".price, .property-price, .asking")
                size_el = card.select_one(".size, .sqft, .area")
                broker_el = card.select_one(".broker, .agent, .contact")
                ...
                self.logger.warning(f"Failed to parse Behar card: {e}")

        return listings


def scrape() -> list[dict]:
    return BeharScraper().scrape()
